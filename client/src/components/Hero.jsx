import React from 'react'
import { useNavigate } from 'react-router-dom'
import { assets } from '../assets/assets'
import { motion } from 'framer-motion'

const Hero = () => {

    const navigate = useNavigate()

    return (
        <motion.div
            initial={{ opacity: 0, y: 40 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, ease: 'easeOut' }}
            className='px-4 sm:px-20 xl:px-32 relative inline-flex flex-col w-full justify-center bg-[url(/gradientBackground.png)] bg-cover bg-no-repeat min-h-screen'
            data-aos="fade-up"
        >
            <div className='text-center mb-6' data-aos="fade-up" data-aos-delay="100">
                <h1 className='text-3xl sm:text-5xl md:text-6xl 2xl:text-7xl font-semibold mx-auto leading-[1.2]'>Create amazing content <br/> with <span className='text-primary'>AI tools</span></h1>
                <p className='mt-4 max-w-xs sm:max-w-lg 2xl:max-w-xl m-auto max-sm:text-xs text-gray-600'>Transform your content creation with our suite of premium AI tools. Write articles, generate images, and enhance your workflow.</p>
            </div>
            <div className='flex flex-wrap justify-center gap-4 text-sm max-sm:text-xs' data-aos="fade-up" data-aos-delay="200">
                <motion.button
                    onClick={()=> navigate('/ai')}
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.97 }}
                    transition={{ type: 'spring', stiffness: 300 }}
                    className='bg-primary text-white px-10 py-3 rounded-lg transition cursor-pointer'
                >Start creating now</motion.button>
                <motion.button
                    whileHover={{ scale: 1.03 }}
                    whileTap={{ scale: 0.98 }}
                    transition={{ type: 'spring', stiffness: 300 }}
                    className='bg-white px-10 py-3 rounded-lg border border-gray-300 transition cursor-pointer'
                >Watch demo</motion.button>
            </div>
            <div className='flex items-center gap-4 mt-8 mx-auto text-gray-600' data-aos="fade-up" data-aos-delay="300">
                <img src={assets.user_group} alt="" className='h-8'/> Trusted by 50+ people
            </div>
        </motion.div>
  )
}

export default Hero
